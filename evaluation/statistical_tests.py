...
class StatisticalAnalyzer:
    ...
    Evaluates whether accuracy gains are statistically significant using paired non-parametric tests.
    """
    ...
    def wilcoxon_test(self) -> Dict[str, float]:
        ...
        # Prevent scipy ValueError if the predictions are completely identical
        if baseline == mnsr:
            return {"statistic": 0.0, "p_value": 1.0}
    ...
    def mcnemar_test(self) -> Dict[str, float]:
        ...
        table = [
            ...
            [mnsr_only, both_wrong]
        ]
        result = mcnemar(table, exact=False, correction=True)
        return {
            "chi_square": round(float(result.statistic), 4),
