...
class BenchmarkReport:
    """
    Generates academic and experimental benchmark statistics from raw MNSR experiment json results.
    """
    ...
    def generate(self) -> Dict[str, Any]:
        ...
        revisions = sum(1 for x in self.results if x.get("action_path") == "REVISE")
        backtracks = sum(1 for x in self.results if x.get("action_path") == "BACKTRACK")
        memory = sum(1 for x in self.results if x.get("action_path") == "RETRIEVE_MEMORY")
        ...
        return {
            ...
            "Total Revision Actions": revisions,
            "Total Backtrack Actions": backtracks,
            "Total Memory Retrievals": memory
        }
    def print_report(self):
        report = self.generate()
        print("\n" + "="*15 + " IEEE BENCHMARK METRICS REPORT " + "="*15 + "\n")
        for k, v in report.items():
            if isinstance(v, float) and "Accuracy" in k or "Delta" in k:
