...
Expanded experiment metrics for MNSR.

WHY THIS CHANGE
---------------
The original metrics tracked only three action counters (revisions,
backtracks, memory_retrievals), matching the old three-action controller.
With ten possible actions and per-step trace logs from the new pipeline,
metrics needs to (a) count every new action type, (b) track reflection
*depth* per question (not just a single `total_steps` sum), and (c)
aggregate confidence/validation-success/memory-retrieval rates required
by the "Evaluation Improvements" spec (Reflection Count, Average
Reflection Depth, Average Confidence, Validation Success Rate, Memory
Retrieval Rate, Correction Success Rate).

RESEARCH NOVELTY
-----------------
Reporting per-action-type frequency alongside accuracy deltas lets the
paper causally connect "which meta-cognitive action fired" to "how much
accuracy did it recover" (Correction Success Rate), rather than a single
aggregate number -- this is the evidence base for the router's
"targeted-strategy" claim in router.py.
"""
