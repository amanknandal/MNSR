...
WHY THIS CHANGE
---------------
`action_path` in results.json can now be any of the ten controller
actions (not just REVISE/BACKTRACK/RETRIEVE_MEMORY), and each log entry
now carries `mnsr_confidence` and `correction_history`. This version
aggregates all action types generically (instead of three hardcoded
counters) and reports average confidence and exact-match accuracy
alongside the existing latency/accuracy figures, matching the
"Evaluation Improvements" spec (Accuracy, Exact Match, Average
Confidence).
"""
...
class BenchmarkReport:
    """
    Generates academic and experimental benchmark statistics from raw
    MNSR experiment json results.
    """
