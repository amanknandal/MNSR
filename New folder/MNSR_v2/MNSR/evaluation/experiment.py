...
WHY THIS CHANGE
---------------
Two additions on top of the original, already-fairly-generic experiment
runner:

1. `dataset_type` inference from the dataset *filename* (gsm8k -> numeric,
   strategyqa/boolq -> boolean, mmlu/arc -> multiple_choice, truthfulqa/
   halueval -> freeform) is threaded into `MNSRPipeline.solve()` so the
   validator/controller can apply dataset-appropriate format constraints
   (spec item 9: MMLU/ARC/BoolQ support + automatic schema normalization).
2. Passing the richer `MNSRPipeline` result (confidence, correction
   history) into `ExperimentMetrics.update()` so the new aggregate metrics
   (avg confidence, correction success rate, memory retrieval rate) are
   actually populated instead of defaulting to zero.

Everything else (dataset loading, schema normalization, answer matching)
is preserved from the original implementation since it already covers a
wide variety of benchmark schemas.
"""
...
class MNSRExperiment:
    ...
    def normalize_sample(self, sample):
        ...
        # Multiple-choice schemas (MMLU/ARC): fold options into the prompt.
    ...
    def process_single_trajectory(self, args):
        ...
        print(f"[{idx+1}/{total_count}] Baseline={baseline_correct} | MNSR={mnsr_correct}")
