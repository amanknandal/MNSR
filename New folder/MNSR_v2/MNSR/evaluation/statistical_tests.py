...
WHY THIS CHANGE
---------------
No behavioral change from the original file -- it already correctly
computes paired non-parametric significance (Wilcoxon, McNemar) purely
from `baseline_correct`/`mnsr_correct` booleans in results.json, which are
still produced with the same names by the updated experiment.py. Kept
verbatim (module docstring added) so downstream tooling (run.py) does not
need any changes to keep working with the expanded pipeline.
"""
...
class StatisticalAnalyzer:
    """
    Performs statistical significance analysis for MNSR experiments.
    Evaluates whether accuracy gains are statistically significant using
